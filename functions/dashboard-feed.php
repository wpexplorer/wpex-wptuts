...
function rc_mdm_register_widgets() {
	...
	wp_add_dashboard_widget('widget_wpexplorer_feed', esc_html__('WordPress Articles', 'rc_mdm'), 'wpex_dashboard_rss_box');
	wp_add_dashboard_widget('widget_wdexplorer_feed', esc_html__('Web Design Articles', 'rc_mdm'), 'wde_dashboard_rss_box');
}
...
function wpex_dashboard_rss_box() {
	...
	foreach ( $my_feeds as $feed) :
		...
		// Check items
		if ( $maxitems == 0 ) {
			echo '<li>'.esc_html__( 'No items', 'wptuts').'.</li>';
		} else {
			// Loop through each feed item and display each item as a hyperlink.
			foreach ( $rss_items as $item ) :
				$item_date = human_time_diff( $item->get_date('U'), current_time('timestamp')).' '.esc_html__( 'ago', 'rc_mdm' );
