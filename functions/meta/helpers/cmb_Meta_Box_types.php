...
class cmb_Meta_Box_types {
	...
	private static function repeat_table_close() {
		echo '</tbody></table><p class="add-row"><a data-selector="', self::$field['id'] ,'_repeat" class="add-row-button button" href="#">'. esc_html__( 'Add Row', 'cmb' ) .'</a></p>';
	}
	...
	private static function repeat_cell( $input ) {
		return '<td>'. $input .'</td><td class="remove-row"><a class="button remove-row-button" href="#">'. esc_html__( 'Remove', 'cmb' ) .'</a></td>';
	}
	...
	public static function file_list( $field, $meta, $object_id ) {
		...
		echo '<input class="cmb_upload_button button cmb_upload_list" type="button" value="'. esc_html__( 'Add or Upload File', 'cmb' ) .'" />', self::desc( $field['desc'], true );
			...
			foreach ( $meta as $id => $fullurl ) {
				if ( self::is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p><a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove Image', 'cmb' ) .'</a></p>
	...
	public static function file( $field, $meta, $object_id, $object_type ) {
		...
		echo '<input class="cmb_upload_button button" type="button" value="'. esc_html__( 'Add or Upload File', 'cmb' ) .'" />';
		...
		'<div id="', $field['id'], '_status" class="cmb_media_status">';
			if ( ! empty( $meta ) ) {
				...
				if ( self::is_valid_img_ext( $meta ) ) {
					...
					echo '<p><a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. esc_html__( 'Remove Image', 'cmb' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo esc_html__( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">'. esc_html__( 'Download', 'cmb' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $field['id'], '">'. esc_html__( 'Remove', 'cmb' ) .'</a>)';
				}
			}
		echo '</div>';
	}
